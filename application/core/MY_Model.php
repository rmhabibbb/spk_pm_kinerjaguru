...
class MY_Model extends CI_Model
{
	...
	public function cekpasslama($nip,$pass){
	...
	public function cekpasslama2($nip,$pass){
		$this->db->where('nip' , $nip);
		$query = $this->db->get($this->data['table_name'])->row();
 
		$output = "";
	  	if ( $query->password != md5($pass)  ) {
	  		$output .= '<i style="color:red">Password lama salah!</i>';
	  	}
	  	return $output;
	}
    ...
    public function cek_password_length2($password)
        {
		  ...
		  $output = "";
          if (strlen($password) < 8 ) {
            $output .= '<i style="color:red">Minimum 8 Karakter!</i>';
          }
          return $output;
        }
        ...
        public function cek_passwords2($password,$confirm_password){
        	 $output = ""; 
            if ( $password != $confirm_password) {
            $output .= '<i style="color:red">Passwords baru tidak sama!</i>';
          }
          return $output;
        }
